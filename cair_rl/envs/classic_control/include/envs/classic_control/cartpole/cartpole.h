...
class CartPoleEnv{
    ...
    const double CART_POS_X_MULTIPLIER = scale + screen_width / 2.0;
...
public:
    ...
    const cv::Mat& render(const char* = "human"/*mode*/){
        ...
        // Set cart position
        cart.x = state[0] * CART_POS_X_MULTIPLIER;
    ...
    const std::tuple<std::array<double, 4>, double, bool, std::unordered_map<std::string, std::string>> step(int action){
        ...
        const auto temp = (force + polemass_length * std::pow(state[3], 2) * sintheta) / total_mass;
        const auto thetaacc = (gravity * sintheta - costheta * temp) / (length * (4.0 / 3.0 - masspole * std::pow(costheta, 2) / total_mass));
        ...
        if(done){
            if(steps_beyond_done == 1){
                spdlog::warn("You are calling 'step()' even though this environment has already returned done = True. "
